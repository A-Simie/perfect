...
export async function POST(request: Request) {
  ...
  const referencePath = feature === "clothing" ? `outfit-${garmentCategory?.replace("_body", "")}-${suffix}.jpg` : feature === "shoes" ? `shoes-${suffix}.jpg` : `necklace-${suffix}.jpg`;

  try {
    const taskId = await startCoordinatedLookTask({ feature, sourceFileId, referencePath, garmentCategory, gender });
